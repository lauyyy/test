...
angular.module("app.subject",["ng"])
    ...
    .controller("subjectController",["$scope","commonService","subjectService","$routeParams","$location"
        ...
        commonService.getAllTypes(function (result) {
            $scope.types = result;
        });
        commonService.getAllDepartment(function (result) {
            $scope.departmentes=result;
        });
        commonService.getAllTopics(function (result) {
            $scope.topics=result;
        });
        commonService.getAllLevel(function (result) {
            $scope.levels=result;
        });
        //获取所有的题目信息
    ...
    /* commonService.getAllSubjectData(function (result) {
     $scope.types=result;
     //$scope.department=result;
     //$scope.topics=result;
     //$scope.level=result;
     });*/
    /*.config(function ($routeProvider/!*,commonServiceProvider*!/) {
        $routeProvider.when("/AddSubject",{
            templateUrl:"tpl/subject/subjectAdd.html",
            controller:"addSubjectController"
        });
        /!*commonServiceProvider.setUrl("data/type.js");*!/
    })*/
   /* .provider("commonService",function () {
        ...
        this.$get=function ($http) {
            var self=this;
            return{
                getAllSubjectData:function (handler) {
                    $http.jsonp(self.url+"?callback=JSON_CALLBACK").success(function (data) {
                        handler(data);
                    });
                    /!*then(function (data) {
                        handler(data);
    ...
    .factory("commonService",["$http",function ($http) {
        return{
            convertIndexToNo:function (index) {
                return index = index==0?'A':(index==1?'B':(index==2?'C':(index==3?'D':'E')))
            },
    ...
    .service("subjectService",["$http","$httpParamSerializer",function ($http,$httpParamSerializer) {
        ...
        this.getAllSubjects=function (params,handler) {
            var data={};
            //循环遍历将data转换成后台能事变的筛选对象
            for(var key in params){
                var val = params[key];
                //只有当val 不等于0的时候，才设置筛选属性
                if(val!=0){
                    switch(key){
                        case "a":
